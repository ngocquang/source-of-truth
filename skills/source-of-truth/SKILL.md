...
- **Parking history in `Notes`.** Why a cross-cutting choice was made belongs in `docs/decisions/`; how a hard bug was found belongs in `docs/debugging/`; `Notes` is for short gotchas. Boundary table → [`references/catalog-format.md`](references/catalog-format.md), `## Where content lives`.
